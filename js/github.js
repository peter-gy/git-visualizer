...
function getRepo(branch) {
  ...
  $.ajax({
    url:
      ...
      "/" +
      repo +
      (branch ? "/branches/" + branch : "/commits"),
...
function update() {
  ...
  node = node
    ...
    .style("fill", function (d) {
      return d.name === "root"
        ...
        ? "#ccc"
        : color(
            d.filename.lastIndexOf(".") >= 0
              ? d.filename.substring(d.filename.lastIndexOf("."))
              : "others"
          );
    });
  ...
  node
    ...
    .on("mouseover", function (d) {
      ...
      link.style("stroke-width", function (l) {
        if (
          (ancestors && d.name == l.target.name) ||
          ancestors.indexOf(l.target.name + "/") >= 0
        )
          return 4;
        else return 2;
      });
  ...
  svg.selectAll("circle.node").each(function (v) {
    $(this).tipsy({
      gravity: "w",
      opacity: 1,
      html: true,
